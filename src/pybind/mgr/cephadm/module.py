...
class HostAssignment(object):
    ...
    def __init__(self,
                 ...
                 get_daemons_func=None,
                 service_type=None,  # type: Optional[str]
                 ):
        assert spec and get_hosts_func and get_daemons_func and service_type
    ...
    def load(self):
        ...
        if len(existing) >= self.spec.placement.count:
            chosen = self.scheduler.place(existing, self.spec.placement.count)
            logger.info('Hosts with existing daemons: {}'.format(chosen))
            self.spec.placement.set_hosts(chosen)
            return self.spec

        need = self.spec.placement.count - len(existing)
    ...
    def pick_candidates(self):
        ...
        candidates = [
            HostPlacementSpec(x, '', '')
            for x in self.get_hosts_func(self.daemon_type)
        ]
        if candidates:
