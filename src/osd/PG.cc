...
void PG::handle_scrub_reserve_request(OpRequestRef op)
{
  ...
  if (op->get_req()->get_type() == MSG_OSD_SCRUB_RESERVE) {
    ...
    Message *reply;
    if (scrubber.reserved) {
      reply = new MOSDScrubReserve(info.pgid, m->map_epoch,
				   MOSDScrubReserve::GRANT, pg_whoami);
    } else {
      reply = new MOSDScrubReserve(info.pgid, m->map_epoch,
				   MOSDScrubReserve::REJECT, pg_whoami);
    }
...
void PG::scrub_reserve_replicas()
{
  ...
  for (set<pg_shard_t>::iterator i = actingbackfill.begin();
    ...
    if (HAVE_FEATURE(get_min_acting_features(), SERVER_LUMINOUS)) {
      osd->send_message_osd_cluster(
	i->osd,
	new MOSDScrubReserve(info.pgid, get_osdmap()->get_epoch(),
