...
protected:
  static void split_by_object(
    ...
    map<hobject_t, osd::list<pg_log_entry_t>, hobject_t::BitwiseComparator> *out_entries) {
    while (!entries.empty()) {
      osd::list<pg_log_entry_t> &out_list = (*out_entries)[entries.front().soid];
  ...
  static void _merge_object_divergent_entries(
    ...
    const osd::list<pg_log_entry_t> &entries, ///< [in] entries for hoid to merge
  ...
  static void _merge_divergent_entries(
    const IndexedLog &log,               ///< [in] log to merge against
    osd::list<pg_log_entry_t> &entries,       ///< [in] entries to merge
    ...
    map<hobject_t, osd::list<pg_log_entry_t>, hobject_t::BitwiseComparator > split;
    split_by_object(entries, &split);
    for (map<hobject_t, osd::list<pg_log_entry_t>, hobject_t::BitwiseComparator>::iterator i = split.begin();
	 i != split.end();
	 ++i) {
      _merge_object_divergent_entries(
	log,
