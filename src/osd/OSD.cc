...
void OSD::dispatch_session_waiting(Session *session, OSDMapRef osdmap)
{
  ...
  while (i != session->waiting_on_map.end()) {
    ...
    if (m->get_type() == CEPH_MSG_OSD_OP) {
      pg_t actual_pgid = osdmap->raw_pg_to_pg(static_cast<MOSDOp*>(m)->get_pg());
...
void OSD::ms_fast_dispatch(Message *m)
{
  ...
  if (m->get_type() != CEPH_MSG_OSD_OP ||
    ...
    // legacy client; we need to map them to an spg_t while preserving
    // delivery order.
...
void OSD::ShardedOpWQ::wake_pg_waiters(spg_t pgid)
{
  ...
  unsigned pushes_to_free = 0;
  {
    ...
    if (p != sdata->pg_slots.end()) {
      dout(20) << __func__ << " " << pgid << " ls " << p->second.ls
	       << " waiting_for_pg=" << (int)p->second.waiting_for_pg << dendl;
      for (auto i = p->second.ls.rbegin(); i != p->second.ls.rend(); ++i) {
	sdata->_enqueue_front(make_pair(pgid, *i), osd->op_prio_cutoff);
      }
      for (auto& q : p->second.ls) {
...
void OSD::ShardedOpWQ::prune_pg_waiters(OSDMapRef osdmap, int whoami)
{
  ...
  for (auto sdata : shard_list) {
    ...
    while (p != sdata->pg_slots.end()) {
      ...
      if (!slot.ls.empty()) {
	if (osdmap->is_acting_osd_shard(p->first, whoami)) {
	...
	while (!slot.ls.empty() &&
	       slot.ls.front().get_map_epoch() <= osdmap->get_epoch()) {
	  dout(20) << __func__ << "  " << p->first
		   << " item " << slot.ls.front()
		   << " epoch " << slot.ls.front().get_map_epoch()
		   << " <= " << osdmap->get_epoch()
		   << ", stale, dropping" << dendl;
	  pushes_to_free += slot.ls.front().get_reserved_pushes();
	  slot.ls.pop_front();
	}
      }
      if (slot.ls.empty() &&
...
void OSD::ShardedOpWQ::clear_pg_waiters()
{
  for (auto sdata : shard_list) {
    Mutex::Locker l(sdata->sdata_op_ordering_lock);
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
  ...
  uint64_t requeue_seq;
  {
    ...
    dout(30) << __func__ << " " << item.first << " ls " << slot.ls
	     << " waiting_for_pg=" << (int)slot.waiting_for_pg << dendl;
    slot.ls.push_back(item.second);
  ...
  if (!pg) {
    ...
    if (osdmap->is_acting_osd_shard(item.first, osd->whoami)) {
      dout(20) << __func__ << " " << item.first
	       << " no pg, should exist, will wait" << " on " << *qi << dendl;
      slot.ls.push_front(*qi);
...
{
  uint32_t shard_index = item.first.pgid.ps() % shard_list.size();
  ShardData* sdata = shard_list[shard_index];
  assert (NULL != sdata);
  sdata->sdata_op_ordering_lock.Lock();
  auto p = sdata->pg_slots.find(item.first);
  if (p != sdata->pg_slots.end() && !p->second.ls.empty()) {
    p->second.ls.push_front(item.second);
    item.second = p->second.ls.back();
    p->second.ls.pop_back();
    dout(20) << __func__ << " " << item.first << " " << p->second.ls.front()
