...
void PGMap::Incremental::write_incremental(MonitorDBStore::Transaction *t, uint64_t features) const
{
...

  {
    ...
    t->put("pgmap_dirty_pgs", version, dirty);
  }
  {
    bufferlist dirty;
    ...
    t->put("pgmap_dirty_osds", version, dirty);
  }
  {
    bufferlist stamp_bl;
    ::encode(stamp, stamp_bl);
    t->put("pgmap_dirty_stamp", version, stamp_bl);
  }
}
...
int PGMap::read_incremental(CephContext *cct, MonitorDBStore *store)
{
  version_t v = version + 1;

  // get dirty lists
  bufferlist dirty_pgs, dirty_osds, stamp_bl;
  int r = store->get("pgmap_dirty_pgs", v, dirty_pgs);
  if (r < 0)
    return r;
  r = store->get("pgmap_dirty_osds", v, dirty_osds);
  if (r < 0)
    return r;
  r = store->get("pgmap_dirty_stamp", v, stamp_bl);
  if (r < 0)
    return r;

  utime_t inc_stamp;
  {
    bufferlist::iterator p = stamp_bl.begin();
    ::decode(inc_stamp, p);
  }
  ...
  while (!p.end()) {
    ...
    bufferlist bl;
    r = store->get("pgmap_pg", stringify(pgid), bl);
  ...
  while (!p.end()) {
    ...
    bufferlist bl;
    r = store->get("pgmap_osd", stringify(osd), bl);
