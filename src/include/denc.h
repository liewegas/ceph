...
/*
  ...
  =============================================
    ...
    struct denc_traits<T> {
      ...
      static constexpr bool bounded = false;
      static constexpr bool featured = false;
      static constexpr bool with_denc = false;
...
struct denc_traits<
  ...
  static constexpr bool with_dencv = true;
  static void bound_encode(const T &o, size_t& p, uint64_t f=0) {
    p += sizeof(T);
  }
...
namespace _denc {
  ...
  template<typename T>
  struct has_legacy_denc<T,
    typename std::enable_if<denc_traits<T>::with_dencv>::type> : std::true_type
  {
    static void decode(T& v, bufferlist::iterator& p) {
...
struct denc_traits<
  ...
  static constexpr bool with_dencv = (a_traits::with_dencv &&
				      b_traits::with_dencv);
  ...
  template<typename AA=A>
  static typename std::enable_if<sizeof(AA) && with_dencv>::type
...
namespace _denc {
  ...
  public:
    ...
    static constexpr bool bounded = false;
    static constexpr bool with_dencv = traits::with_dencv;
...
#define _DECLARE_CLASS_DENC(T, b)					\
  template<> struct denc_traits<T> {					\
    ...
    static constexpr bool with_dencv = _denc::has_legacy_denc<T>::value;\
