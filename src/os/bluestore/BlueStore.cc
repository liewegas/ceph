                                  ...
                                  bool force)
{
  ...
  if (onode->onode.extent_map_shards.empty()) {
    ...
    struct dirty_shard_t {
      BlueStore::ExtentMap::Shard *shard = nullptr;
      bufferlist bl;
  ...
  KeyValueDB::Transaction t)
{
  ...
  if (sv.empty()) {
    ...
    dout(20) << __func__ << " checking spanning blobs" << dendl;
  ...
  uint32_t length)
{
  ...
  while (start <= last) {
    ...
    if (!p->loaded) {
      bufferlist v;
      generate_extent_shard_key_and_apply(onode->key, p->shard_info->offset,
					  &key,
        [&](const string& final_key) {
      ...
      dout(20) << __func__ << " open shard 0x" << std::hex
	       << p->shard_info->offset << std::dec << " (" << v.length()
	       << " bytes)" << dendl;
